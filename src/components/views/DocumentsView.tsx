...
export const DocumentsView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (alive) addToast(e instanceof Error ? e.message : 'Failed to load documents', 'error');
      } finally {
  ...
  const pipelineSteps: PipelineStep[] = [
    'Validation', 'Parsing', 'Metadata Extraction', 'Chunking', 'Versioning', 'Approved Repository',
  ];
              ...
              {filteredDocs.map(doc => (
                <tr key={doc.id} className="hover:bg-purple-950/20 transition group">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap font-mono text-xs text-slate-300">
                    ...
                    <div className="text-slate-500 text-[10px]">{(doc as DocumentItem & { file_type?: string; fileType?: string }).file_type || (doc as DocumentItem & { fileType?: string }).fileType || ''}</div>
                  </td>
